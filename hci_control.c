...
#include "string.h"
#define HCI_BLE_HID_DEV_KEY_STORAGE_VS_ID   0x200
/*****************************************************************************
**  Constants
*****************************************************************************/
#define KEY_INFO_POOL_BUFFER_SIZE   145
...
typedef struct
{
    ...
    hidh_le_gatt_cache_t  ble_hidh_gatt_cache;
...
int hci_control_nvram_alloc(void);
void hci_control_nvram_delete(int nvram_id, wiced_bool_t from_host);
int hci_control_nvram_write(uint8_t data_len, uint8_t *p_data, BOOLEAN from_host);
int hci_control_nvram_find_id(uint8_t *p_data, int len);
int hci_control_nvram_write_ble_hidh_gatt_cache(wiced_bt_device_address_t bdaddr, hidh_le_gatt_cache_t *p_gatt_cache, wiced_bool_t send_to_host);
int hci_control_nvram_read_ble_hidh_gatt_cache( wiced_bt_device_address_t bdaddr, hidh_le_gatt_cache_t *p_gatt_cache);
...
extern const wiced_transport_cfg_t transport_cfg_hid;
/******************************************************
...
const wiced_transport_cfg_t transport_cfg_hid =
{
		.type = WICED_TRANSPORT_UART,
		.cfg = {{ WICED_TRANSPORT_UART_HCI_MODE, HCI_UART_DEFAULT_BAUD}},
		.rx_buff_pool_cfg  ={TRANS_UART_BUFFER_SIZE, 2},
...
APPLICATION_START( )
{
    ...
    wiced_transport_init(&transport_cfg_hid);
        ...
        wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    switch( event )
    {
        ...
        case BTM_ENCRYPTION_STATUS_EVT:
            ...
            hidh_le_encryption_changed(&p_event_data->encryption_status);
        ...
        case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
			...
			if ( ( nvram_id = hci_control_nvram_find_id(p_event_data->paired_device_link_keys_update.bd_addr, BD_ADDR_LEN ) ) == 0)
			{
				// This is the first time, allocate id for the new memory chunk
				nvram_id = hci_control_nvram_alloc();
				WICED_BT_TRACE( "Allocated NVRAM ID:%d\n", nvram_id );
			}
			bytes_written = hci_control_nvram_write( sizeof( wiced_bt_device_link_keys_t ),
					( uint8_t* ) &p_event_data->paired_device_link_keys_update, WICED_FALSE );
        ...
        case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
			...
			if ( ( nvram_id = hci_control_nvram_find_id(
...
int hci_control_nvram_write_ble_hidh_gatt_cache(wiced_bt_device_address_t bdaddr,
        hidh_le_gatt_cache_t *p_gatt_cache, wiced_bool_t send_to_host)
{
    int nvram_id;
    int bytes_nb;
    hci_control_nvram_data_t nvram_data;
    ...
    nvram_id = hci_control_nvram_find_id(bdaddr, BD_ADDR_LEN);
    ...
    bytes_nb = hci_control_read_nvram(nvram_id, &nvram_data, sizeof(nvram_data));
    ...
    /* Write it back */
    bytes_nb = hci_control_nvram_write(sizeof(nvram_data), ( uint8_t* ) &nvram_data, WICED_FALSE);
...
int hci_control_nvram_read_ble_hidh_gatt_cache( wiced_bt_device_address_t bdaddr,
        hidh_le_gatt_cache_t *p_gatt_cache)
{
    int nvram_id;
    int bytes_nb;
    hci_control_nvram_data_t nvram_data;
...
void hci_control_nvram_send_to_host(int nvram_id)
{
    ...
    /* Read the current NVRAM data */
    nb_bytes = hci_control_read_nvram(nvram_id, p, sizeof(hci_control_nvram_data_t));
...
//    	hidh_le_remove(hci_control_cb.nvram_chunk[i]->data.link_key.bd_addr);
//
...
wiced_bt_ble_address_type_t hci_control_get_address_type(wiced_bt_device_address_t bdaddr)
{
    ...
    {
        /* Read NVRAM info of this device */
        bytes_read = hci_control_read_nvram( nvram_id, &device_link_key, sizeof( device_link_key ) );
