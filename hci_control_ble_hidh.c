...
static void hci_control_ble_hidh_cback( hidh_le_event_t event,
		hidh_le_event_data_t *p_data );
/*
 * Local functions
 */
...
{
	hidh_le_init(hci_control_ble_hidh_cback);
}
...
void hci_control_ble_hidh_handle_command( uint16_t cmd_opcode, uint8_t* p_data, uint32_t data_len )
{
    ...
    hidh_le_gatt_cache_t gatt_cache;
    int gatt_cache_size;
    ...
    case HCI_CONTROL_HIDH_COMMAND_CONNECT:
        if (data_len == sizeof(bdaddr))
        {
            ...
            {
                status = HIDH_STATUS_ERROR;
            }
            else
            {
                /* Open the BLE HID Connection */
                status = hidh_le_connect(bdaddr, addr_type);
            }
        }
        else
    ...
    case HCI_CONTROL_HIDH_COMMAND_DISCONNECT:
        if (data_len == sizeof(handle))
        {
            ...
            status = hidh_le_disconnect(handle);
        }
        else
            status = HIDH_STATUS_INVALID_PARAM;
        break;
        ...
        if (data_len == sizeof(bdaddr))
        {
            ...
            else
            {
                ...
                if (gatt_cache_size != sizeof(gatt_cache))
                {
                    ...
                    status = hidh_le_add(bdaddr, addr_type, NULL);
                }
                else
                {
                    /* Add the BLE HID Device. We will be able to receive the HID Reports immediately. */
                    status = hidh_le_add(bdaddr, addr_type, &gatt_cache);
                }
            }
        }
        else
    ...
    case HCI_CONTROL_HIDH_COMMAND_GET_DESCRIPTOR:
        if (data_len == sizeof(handle))
        {
            ...
            status = hidh_le_get_descriptor(handle);
        }
        else
            status = HIDH_STATUS_INVALID_PARAM;
        break;
    ...
    case HCI_CONTROL_HIDH_COMMAND_SET_REPORT:
                         ...
                         sizeof(report_id))
        {
            ...
            status = hidh_le_set_report(handle, report_type, report_id, p_data,
                    data_len - 5);
        }
        else
            status = HIDH_STATUS_INVALID_PARAM;
    ...
    case HCI_CONTROL_HIDH_COMMAND_GET_REPORT:
                ...
                sizeof(buffer_size))
        {
            ...
            status = hidh_le_get_report(handle, report_type, report_id, buffer_size);
        }
        else
            status = HIDH_STATUS_INVALID_PARAM;
        break;
    ...
    case HCI_CONTROL_HIDH_COMMAND_SET_PROTOCOL:
        if (data_len == (sizeof(handle)) + sizeof(protocol))
        {
            ...
            status = hidh_le_set_protocol(handle, protocol);
        }
        else
            status = HIDH_STATUS_INVALID_PARAM;
        break;
    ...
    case HCI_CONTROL_HIDH_COMMAND_WAKEUP_PATTERN_SET:
        if (data_len >= (sizeof(bdaddr) + sizeof(wakeup_cmd) + sizeof(report_id)))
        {
            ...
            status = hidh_le_wakeup_pattern_set(bdaddr,
    ...
    case HCI_CONTROL_HIDH_COMMAND_WAKEUP_CONTROL:
        if (data_len >= (sizeof(wakeup_enable) + sizeof(wakeup_gpio) + sizeof(wakeup_polarity)))
        {
            ...
            /* Set the BLE HID WakeUp Pattern */
            status = hidh_le_wakeup_pattern_control(wakeup_enable, wakeup_gpio,
		...
		hidh_le_event_data_t *p_data )
{
    ...
    case HIDH_DESCRIPTOR_EVT:
        ...
        if (p_buf == NULL)
        {
            ...
            UINT8_TO_STREAM(p, HIDH_STATUS_MEM_FULL);
