# LE HID Host Sample application

## Overview

The LE HID Host application is a single chip SoC implementation. It provides a turnkey solution using on-chip keyscan HW component. It operates in Bluetooth LE Mode using HID over GATT Profile (HOGP).

During initialization the app registers with Bluetooth LE Stack and the AIROC™ HID Host Library to  provide LE scanning and HID Host commands for connection management, trusted-device management, HID descriptor access, report exchange, protocol selection, and wake-up pattern configuration.
...
- Persistent BLE HID GATT-cache and bonding-data handlig


- Reading HID descriptors
- Setting HID repots

- Selecting the HID boot or report protcol

- Configuring HID wake-up paterns
- BTSDK HCI trace out

## On startup this demo:
- Initializes the Bluetooth sub system
- Receives NVRAM information from the host

## Instructions
To demonstrate the app, work through the following steps:

1. Plug in the AIROC™ board or keyboard HW into your computer.
...
4. Route to the "HID Host" tab and click on "Start LE Discovery" button to scan for nearby Bluetooth LE HID devices..

5. Every button press on the connected LE HID Device will generate a corresponding report on the LE HID Host device
